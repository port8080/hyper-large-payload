...
# 0. Get public hostname
PUBLIC_HOSTNAME=$( curl -s http://169.254.169.254/latest/meta-data/public-hostname )
echo "This instance's public hostname is: $PUBLIC_HOSTNAME"
...
openssl req -newkey rsa:4096 -nodes -keyout server-key.pem -out server-req.pem -subj "/C=US/CN=$PUBLIC_HOSTNAME"

# 3. Use CA's private key to sign web server's CSR and get back the signed certificate
printf "subjectAltName=DNS:%s\n" "$PUBLIC_HOSTNAME" > server-ext.cnf
